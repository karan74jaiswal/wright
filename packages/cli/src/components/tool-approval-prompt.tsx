...
export function ToolApprovalPrompt({
  ...
  const options = useMemo(() => {
    const opts: Array<{
      label: string;
      value: "allow_once" | "allow_session" | "allow_system" | "deny";
      wildcard?: string;
    }> = [];

    opts.push({ label: "Yes, allow once", value: "allow_once" });
    opts.push({
      label: "Yes, allow for this session (strict match)",
      value: "allow_session",
    });
    opts.push({
      label: "Yes, allow permanently (strict match)",
      value: "allow_system",
    });
    ...
    if (
      ...
      const dir = pendingApproval.target.substring(
        0,
        pendingApproval.target.lastIndexOf("/"),
      );
      if (dir) {
        wildcard = `${dir}/*`;
    ...
    if (wildcard && wildcardDesc) {
      opts.push({
        label: `Yes, for this session (${wildcardDesc})`,
        value: "allow_session",
        wildcard,
      });
      opts.push({
        label: `Yes, permanently (${wildcardDesc})`,
        value: "allow_system",
        wildcard,
      });
    }
  ...
  return (
    ...
    >
      ...
      <box paddingBottom={1} flexDirection="column">
        ...
        <text fg={colors.info}>Target: {pendingApproval.target}</text>
      </box>
